...
def logout():
    ...
    flash("You have successfully logged out your account", "success")
...
def edit_user():
    ...
    if form.validate_on_submit():
        if User.authenticate(username=user.username, password=form.password.data):
            #get the database object using the id stored in the global parameter
            current_user = User.query.get(user.id)
            
            #updating the values of the database object
            ...
            flash("Incorrect password. Edit profile details failed", "danger")
            return redirect(f"/")
